...
/*
 * GPL Vault: activates ACF PRO and silences its licence prompts. Additive filters
 * only — no ACF source file is modified. The stored licence carries the site's live
 * home URL, so it stays valid on any domain and after a URL change. Keep the licence
 * status "active" wherever it is read; the checks below all key off that single state.
 * Remove down to "END GPL Vault" to restore upstream behaviour.
 */
add_filter(
	'pre_option_acf_pro_license',
	function () {
		$home_url = function_exists( 'acf_get_home_url' ) ? acf_get_home_url() : home_url();
...
add_filter(
	'pre_option_acf_pro_license_status',
	function () {
		return array(
			'status'                  => 'active',
			'created'                 => 0,
...
add_filter(
	...
	function ( $preempt, $parsed_args, $url ) {
		// Only handle the ACF activation server, and only its known endpoints;
		// everything else passes through. Never let the fall-through fabricate a response.
		if ( false === strpos( $url, 'connect.advancedcustomfields.com' ) ) {
			return $preempt;
		}
		...
		if ( false !== strpos( $url, '/plugins/activate' ) ) {
			$body = array(
		...
		} elseif ( false !== strpos( $url, '/plugins/validate' ) ) {
			$body = array(
				'expiration'     => YEAR_IN_SECONDS,
				'license_status' => $active_status,
				'status'         => 1,
			);
		} elseif ( false !== strpos( $url, '/plugins/deactivate' ) ) {
			$body = array(
				'message' => 'Licence key deactivated.',
				'status'  => 1,
			);
		} elseif ( false !== strpos( $url, '/plugins/update-check' ) || false !== strpos( $url, '/plugins/get-info' ) ) {
			// Answer locally so the site is never reported to the activation server.
			// Updates are handled separately, so report no updates from here.
			$body = array(
				'plugins'   => array(),
				'no_update' => array(),
			);
